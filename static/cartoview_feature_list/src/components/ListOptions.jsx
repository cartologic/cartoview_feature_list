...
export default class ListOptions extends Component {
	constructor( props ) {
		...
		this.state = {
			...
			selectedLayer: this.props.config
				...
				: null,
			selectedAttribute: this.props.config
	...
	selectAttribute( ) {
		if ( this.refs.selectedAttribute.value !== "" ) {
			this.setState({ selectedAttribute: this.refs.selectedAttribute.value })
		}
	}
	loadAttributes( ) {
		let typename = this.refs.selectedLayer
			? this.refs.selectedLayer.value
